...
export default function AssignmentBlockTeacher(
	props: IAssignmentBlockTeacherProps
) {
	...
	async function handleAttemptAssignment() {
		if (data?.id)
			try {
				const response = await attemptAssignment({
					assignment_id: data?.id,
				}).unwrap();
				console.log("response:: ", response);
				navigate(
					`/courses/assignments/${data?.id}/${response?.data?.id}`
				);
			} catch (error: any) {
	...
	async function handleViewAttemptsByAssignment() {
		if (data?.id) {
			navigate(
				`/teacher/courses/${courseId}/assignments/${data?.id}/attempts`
			);
		} else {
			toast.error("Something went wrong!");
		}
	...
	return (
        ...
        > */}
			...
			<div className={cx("info")}>
				<Row gutter={12}>
					...
					<Col span={24}>
						<SimpleInfo
							label="Độ khó"
							value={<Tag color="blue">Trung bình</Tag>}
						/>
					</Col>
					{/* <Col span={12}>
            <Tag color='green'>14/23 hoàn thành</Tag>
					...
					{mode === ROLE.STUDENT && (
						<Col span={24}>
							<Button onClick={handleAttemptAssignment}>
								Attempt
							</Button>
						</Col>
					)}
					{mode === ROLE.TEACHER && (
						<Col span={24}>
							<Button onClick={handleViewAttemptsByAssignment}>
								View all attempts
							</Button>
						</Col>
					)}
